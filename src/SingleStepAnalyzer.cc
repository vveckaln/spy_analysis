...
void SingleStepAnalyzer::process(bool isData, urlCodes urlCode, TString path, TString outhistograms, vector<TString> & keys, uint ttbarLike ){
  ...
  init_spy(urls,xsec,true,LUM_,1); // No need to differentiate between channels. Will differentiate channels in the code (faster: only one access to event vars per event)
...
void SingleStepAnalyzer::eventAnalysis(bool newPhys,
				    TString myKey,
  ...
  // pileup reweighting /////////////////////////////////////////////////////////////////////////
  if(! isData_ && false ){ // FIXME DEACTIVATE 
  ...
  */
  ...
  // FIXME: Must implement a switcher in case we want to have different sets of spyfiles at different cutflow steps.
  //        For now, only tauDileptonOSAnalysis is activated.
  tauDileptonOSAnalysis(lepReq, vertices, muons,m_init,electrons,e_init,taus,t_afterLeptonRemoval,jets,j_final,numbJets,junc,jerFactors,myKey,ev, mets);

}


void SingleStepAnalyzer::tauDileptonOSAnalysis(
  bool lepReq,
  ...
  if( ! lepReq ){ return; } // When processing spyfiles, it should never return. Perhaps a cout check about that would be nice :P
  ...
  if( APPLY_MT_CUT_ && mt<MT_CUT_){ return; } 
  ...
  // events at the starting selection step (which for spyfiles is OS) /////////////////////////////////////////////////////////////////////////////////////////////
...
void SingleStepAnalyzer::fillTauDileptonObjHistograms(
...
void SingleStepAnalyzer::fillDebugHistograms(
  TString key, 
  vector<int> & m_v, vector<PhysicsObject> & muons, 
  vector<int> & e_v, vector<PhysicsObject> & electrons,
...
void SingleStepAnalyzer::newPhysics(
...
void SingleStepAnalyzer::getBtagEfficiencies(){
...
pair<double, pair<double,double> >SingleStepAnalyzer::getNbtags( std::vector<PhysicsObject> & jets, vector<int>  & j_v){
...
pair< double,double > SingleStepAnalyzer::get2Nbtags( std::vector<PhysicsObject> & jets, vector<int>  & j_v){
...
void SingleStepAnalyzer::computeMHT( std::vector<PhysicsObject> & jets,  vector<int>  & jetsForMHT, PhysicsObject & lepton ){
...
void SingleStepAnalyzer::computeMHTb(  std::vector<PhysicsObject> & jets,  vector<int>  & jetsForMHT ){
...
/*
bool SingleStepAnalyzer::jetMatchCondition( needs junc , std::vector<PhysicsObject> & jets, vector<int>  & selectedjets, int excluded ){
...
double SingleStepAnalyzer::effOnMatchCondition( needs junc, std::vector<PhysicsObject> & jets, vector<int>  & selectedjets, int excluded ){
...
pair<double,double> SingleStepAnalyzer::getEfficiencyAndError(double pt,double A0,double errA0, double A1, double errA1,double A2, double errA2){
...
pair<double,double> SingleStepAnalyzer::getEfficiencyAndError( std::vector<PhysicsObject> & jets,  vector<int>  & j_final, JetCorrectionUncertainty * junc, vector<double> & jerFactors ){
...
void SingleStepAnalyzer::setSelectionParameters(){
...
void SingleStepAnalyzer::evaluatePDFUncertainty(){
...
pair<double,double> SingleStepAnalyzer::btagSF(double jetPt){
